...
void WebServer::Start() {
    ...
    while(!isClose_) {
        ...
        int eventCnt = epoller_->Wait(timeMS);//调用 epoll_wait() 监听事件，返回发生的事件数量 eventCnt。
        for(int i = 0; i < eventCnt; i++) {
            /* 处理事件 */
            int fd = epoller_->GetEventFd(i);
            uint32_t events = epoller_->GetEvents(i);//事件类型（可读、可写、错误等）。
            if(fd == listenFd_) {
                DealListen_();
            }
            else if(events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR)) { //EPOLLRDHUP：对端 关闭连接（收到 FIN） EPOLLHUP：连接 挂起，可能是异常断开 EPOLLERR：发生 错误（连接异常） CloseConn_(&users_[fd]) 关闭该 fd 对应的客户端连接，并从 epoll 监听中移除
                assert(users_.count(fd) > 0);
                CloseConn_(&users_[fd]);
            }
            else if(events & EPOLLIN) { //EPOLLIN 事件 说明 客户端发送了数据（HTTP 请求）
                assert(users_.count(fd) > 0);
                DealRead_(&users_[fd]);
            }
            else if(events & EPOLLOUT) {//EPOLLOUT 事件 说明 客户端可写（服务器可以发送 HTTP 响应）
                assert(users_.count(fd) > 0);
